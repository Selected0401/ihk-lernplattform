# Digistore24 Produkttexte und E-Mail-Vorlagen

## Produktname

IHK 80/20 Prüfungstrainer Büromanagement

## Kurzbeschreibung

Eigene prüfungsnahe Übungsplattform für Kaufleute für Büromanagement: Trainiere typische Anforderungen der Zwischenprüfung mit interaktiven Office-Aufgaben, Musterlösungen und einem klaren 80/20-Lernsystem.

## Rechtssichere Produktbeschreibung

Der IHK 80/20 Prüfungstrainer hilft Auszubildenden und Umschülern im Ausbildungsberuf Kaufmann/Kauffrau für Büromanagement, typische Office-Aufgabentypen der Zwischenprüfung gezielt zu üben. Enthalten sind eigene, prüfungsnahe Übungsaufgaben zu Excel, Word, PowerPoint und Outlook, klare Musterlösungen und ein fokussierter Lernaufbau für wenig Lernzeit.

Dieses Angebot steht in keiner Verbindung zur Industrie- und Handelskammer oder zu einer offiziellen Prüfungsstelle. Es werden keine originalen IHK-Prüfungen oder offiziellen Prüfungsunterlagen bereitgestellt. Alle Aufgaben, Texte und Lösungen wurden eigenständig erstellt und dienen der prüfungsnahen Vorbereitung.

## Preisvarianten

### Launch-Angebot
- Name: Prüfungstrainer Pro Launch
- Preis: 99€ einmalig
- Enthält: Plattformzugang, 56 Aufgaben, interaktive Lernumgebung, Musterlösungen

### Regulär / Plus
- Name: Prüfungstrainer Plus
- Preis: 149€ einmalig
- Enthält: Plattformzugang plus Bonusmaterial, Lernplan, zusätzliche Simulationen oder Prüfungsangst-Checkliste

## Checkout-Bullets

- Einmalzahlung, kein Abo
- Zugangscode per E-Mail nach Kauf
- Browser- und iPhone-taugliche Lernplattform
- 56 eigene prüfungsnahe Aufgaben
- Excel, Word, PowerPoint und Outlook trainieren
- 80/20-System für maximalen Lernerfolg bei wenig Zeit
- Ideal bei Prüfungsangst und Lernchaos

## Auslieferungs-E-Mail

Betreff: Dein Zugangscode zum IHK 80/20 Prüfungstrainer

Hallo,

vielen Dank für deinen Kauf des IHK 80/20 Prüfungstrainers.

Dein Zugangscode lautet:

{{ZUGANGSCODE}}

Login-Link:
https://selected0401.github.io/ihk-lernplattform/login.html

So startest du:
1. Öffne den Login-Link.
2. Gib deinen Zugangscode ein.
3. Starte mit dem Modul, das dir aktuell am meisten Punkte bringt.
4. Wenn du auf dem iPhone lernst: Öffne die Seite in Safari und füge sie zum Home-Bildschirm hinzu.

Wichtig: Dieses Angebot steht in keiner Verbindung zur Industrie- und Handelskammer oder zu einer offiziellen Prüfungsstelle. Alle Aufgaben sind eigenständig erstellt und dienen der prüfungsnahen Vorbereitung.

Viel Erfolg beim Lernen!

Support: DEINE-SUPPORT-EMAIL-HIER

## Support-Vorlage: Code funktioniert nicht

Betreff: Zugangscode funktioniert nicht

Hallo,

bitte prüfe zuerst, ob der Code vollständig und ohne Leerzeichen eingegeben wurde. Falls es weiterhin nicht funktioniert, antworte bitte mit:

- deiner Kauf-E-Mail
- deinem Zugangscode
- Screenshot der Fehlermeldung

Wir prüfen den Zugang dann manuell.

## Digistore24 Danke-Seite

Danke für deinen Kauf. Dein Zugangscode wird dir per E-Mail zugesendet. Falls du keine E-Mail findest, prüfe bitte den Spam-Ordner.

Login-Link:
https://selected0401.github.io/ihk-lernplattform/login.html

Danke-Seite:
https://selected0401.github.io/ihk-lernplattform/danke.html
